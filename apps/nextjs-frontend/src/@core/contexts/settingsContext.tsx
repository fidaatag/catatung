...
type Props = {
  ...
  readonly settingsCookie: Settings | undefined;
  readonly mode?: Mode;
};

// Initial Settings Context
export const SettingsContext = createContext<SettingsContextProps | undefined>(null);
...
export function SettingsProvider(props: Props) {
  ...
  const [settingsCookie, updateSettingsCookie] = useObjectCookie<Settings>(
    ...
    JSON.stringify(props.settingsCookie) === '{}' ? updatedInitialSettings : props.settingsCookie,
  );

  // State
  const [_settingsState, _updateSettingsState] = useState<Settings>(
    JSON.stringify(settingsCookie) === '{}' ? updatedInitialSettings : settingsCookie,
  );
  ...
  const updatePageSettings = (settings: Partial<Settings>): (() => void) => {
    ...
    // Returns a function to reset the page settings
    return () => {
      updateSettings(settingsCookie, {updateCookie: false});
    };
  };
