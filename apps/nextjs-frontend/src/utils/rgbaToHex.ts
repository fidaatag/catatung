export const rgbaToHex = (colorStr: string, forceRemoveAlpha = false) => {
  ...
  if (hasSlash) {
    ...
    const [red, green, blue, alpha] = rgbaValues.slice(1, 5).map(Number.parseFloat);
  ...
  return (
    '#' +
    colorStr
      .replaceAll(/^rgba?\(|\s+|\)$/g, '') // Get's rgba / rgb string values
